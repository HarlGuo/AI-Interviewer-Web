...
export default function ReportScreen() {
  ...
  const generate = async () => {
    if (!session?.answers.length || loading) return; setLoading(true); setError('');
    try { await saveReport(await interviewGateway.report(session)); }
    catch (cause) { setError(cause instanceof Error ? cause.message : '请检查网络、后端和 DeepSeek 配置。'); }
    finally { setLoading(false); }
  };
  useEffect(() => { if (session?.answers.length && !report && !requested.current) { requested.current = true; void generate(); } }, [session, report]);
  useEffect(() => { if (report) track('report_viewed', { overall_score: report.overall_score, completed: session?.status === 'completed' }); }, [report]);
