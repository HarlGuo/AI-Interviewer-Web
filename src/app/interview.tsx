...
export default function InterviewScreen() {
  ...
  const toggleSpeechRecognition = async () => {
    try {
      ...
      answerBeforeSpeechRef.current = answerRef.current; finalSpeechRef.current = ''; keepListeningRef.current = true; pendingAutoSubmitRef.current = false; speechUsedRef.current = true;
      startRecognizer();
  ...
  const submit = async () => {
    ...
    try {
      const result = await interviewGateway.turn(session, state.resume, submittedAnswer, source);
      keepListeningRef.current = false; ExpoSpeechRecognitionModule.abort();
      await applyInterviewTurn(submittedAnswer, source, result.next_question, result.completed); setDecisionNote(result.decision_reason); resetAnswerInput(); speechUsedRef.current = false;
